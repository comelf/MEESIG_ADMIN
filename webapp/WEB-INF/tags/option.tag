...
<c:forEach var="block" items="${item.optionBlocks}" varStatus="b">
	<div class="form-group">
		<div class="form-inline">
	 		...
	 		<input type="button" class="btn btn btn-warning" value="항목삭제" onclick="ITEM.delItemOption(this)"/>
	 	</div>
		<c:forEach var="row" items="${block.optionRow}" varStatus="r">
			<div class="form-inline">
				...
				<input type="button" class="btn btn btn-warning" value="삭제" onclick="ITEM.delOptRow(this)"/>
			</div>
		</c:forEach>
			<input type="button" class="btn btn-primary" id="adddpo" value="옵션추가" onclick="ITEM.addOptRow(this)"/>
	</div>		
</c:forEach>
<input type="button" class="btn btn-success" id="adddpo" value="새로운항목" onclick="ITEM.addItemOption(this)"/>
