...
import com.meesig.model.Bundle_Delivery;
...
//			@Result(property="delivery", column="bundle_id", javaType=Bundle_Delivery.class, one=@One(select=("selectBundleDeliveryByBundleId")))
//	})
//	List<OrderBundle> selectOrderBundleByOrderId(@Param("order_id")long order_id);
	...
	@Results(value ={
			...
			@Result(property="orderMenus", column="bundle_id", javaType=List.class, many=@Many(select=("selectOrderMenuByOrderBundleId"))),
			@Result(property="delivery", column="orders_order_id", javaType=Bundle_Delivery.class, one=@One(select=("selectOrderDeliveryByOrderId")))
	})
	...
	@Select("select * from bundle_delivery where orders_order_id = #{order_id}")
	Bundle_Delivery selectBundleDeliveryByBundleId(@Param("order_id")long order_id);
	...
	@Results(value ={
			...
			@Result(property="sumOfDays", column="shop_id", javaType=List.class, many=@Many(select=("selectSumItemsByDays")))
	})
	List<ShopOrders> selectAllShopWithItems();
	...
	@Select("select sum(m.om_item_count) as sum, b.bundle_reserve_date as reserve_date  from order_menu as m "
			...
			+ "where b.bundle_state = 3 and b.shops_shop_id = #{shop_id} group by b.bundle_reserve_date")
	List<SumOfDays> selectSumItemsByDays(@Param("shop_id")int shop_id);
